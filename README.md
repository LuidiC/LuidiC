<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:0a192f,100:112240&height=200&section=header&text=Luidi%20Cadete%20Silva&fontSize=48&fontColor=64ffda&fontAlignY=38&desc=Analista%20de%20Desenvolvimento%20de%20Sistemas%20%7C%20Engenharia%20de%20Software&descAlignY=58&descSize=16&descColor=8892b0" />

<br>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0a66c2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/luidicadetesilva/)
[![GitHub](https://img.shields.io/badge/GitHub-0a192f?style=for-the-badge&logo=github&logoColor=64ffda)](https://github.com/LuidiC)
[![E-mail](https://img.shields.io/badge/E--mail-0a192f?style=for-the-badge&logo=gmail&logoColor=64ffda)](mailto:luidisilva1706@gmail.com)

</div>

---

## 🚀 Sobre mim

Sou **Analista de Desenvolvimento de Sistemas** na **Sicoob Credipel**, onde estruturei e lidero a área de desenvolvimento da cooperativa. Meu foco é projetar e implementar soluções de **automação de processos (RPA)** e sistemas internos que elevam a eficiência operacional da organização.

Vindo de um background em **Business Intelligence**, trago uma visão analítica para a engenharia: entendo o impacto real de cada solução no negócio, não só o código que a resolve. Cursando **Engenharia de Software** na PUC Minas, aprofundo continuamente meu stack técnico com foco em Python, dados e arquitetura de sistemas.

> *"Tecnologia bem aplicada transforma processos e potencializa resultados."*

---

## 🛠️ Stack Técnico

### Linguagens & Desenvolvimento
![Python](https://img.shields.io/badge/Python-0a192f?style=for-the-badge&logo=python&logoColor=64ffda)
![SQL](https://img.shields.io/badge/SQL-0a192f?style=for-the-badge&logo=postgresql&logoColor=64ffda)
![Git](https://img.shields.io/badge/Git-0a192f?style=for-the-badge&logo=git&logoColor=64ffda)
![GitHub](https://img.shields.io/badge/GitHub-0a192f?style=for-the-badge&logo=github&logoColor=64ffda)

### Dados, APIs & Automação
![FastAPI](https://img.shields.io/badge/FastAPI-0a192f?style=for-the-badge&logo=fastapi&logoColor=64ffda)
![Pandas](https://img.shields.io/badge/Pandas-0a192f?style=for-the-badge&logo=pandas&logoColor=64ffda)
![Power Automate](https://img.shields.io/badge/Power%20Automate-0a192f?style=for-the-badge&logo=microsoftpowerautomate&logoColor=64ffda)
![Power BI](https://img.shields.io/badge/Power%20BI-0a192f?style=for-the-badge&logo=powerbi&logoColor=64ffda)
![Docker](https://img.shields.io/badge/Docker-0a192f?style=for-the-badge&logo=docker&logoColor=64ffda)

---

## 💼 Experiência Profissional

**Analista de Desenvolvimento de Sistemas** · Sicoob Credipel
*2024 - Atual*
- Estruturação e liderança técnica da área de desenvolvimento da cooperativa, criada do zero
- Desenvolvimento de soluções de automação de processos internos (RPA) com Python e Power Automate
- Integração entre sistemas e ferramentas internas para ganho de escala operacional

**Analista de Business Intelligence Jr.** · Sicoob Credipel
*2023 - 2024*
- Construção de dashboards estratégicos e modelagem de dados (ETL) em Power BI
- Análise exploratória de dados para suporte à tomada de decisão da diretoria
- Automação de relatórios e pipelines integrando Excel e Power Automate

---

## 🎓 Formação & Certificações

| | Título | Status |
|---|---|---|
| 🎓 | Engenharia de Software — PUC Minas | ▶️ Em andamento · 5º Período |
| 🏆 | Melhor Trabalho do Semestre — PUC Minas | ✅ Reconhecimento Acadêmico |
| 📊 | Power BI Avançado | ✅ Certificado |
| ⚙️ | Power Automate Avançado | ✅ Certificado |
| 💻 | Lógica de Programação | ✅ Certificado |
| 📋 | Excel Avançado | ✅ Certificado |
| 🌐 | Inglês — Cambridge Assessment | ✅ Certificado |

---

## 📌 Projetos em Destaque

### ⚙️ Automação de Processos — Sicoob Credipel
> Soluções desenvolvidas para automação de processos internos da cooperativa, eliminando tarefas manuais e aumentando a eficiência operacional. Integração entre sistemas legados, APIs e ferramentas Microsoft através de Python e Power Automate.

![Python](https://img.shields.io/badge/Python-0a192f?style=flat-square&logo=python&logoColor=64ffda)
![Power Automate](https://img.shields.io/badge/Power%20Automate-0a192f?style=flat-square&logo=microsoftpowerautomate&logoColor=64ffda)
![SQL](https://img.shields.io/badge/SQL-0a192f?style=flat-square&logo=postgresql&logoColor=64ffda)

---

### 🏗️ [Projetos Acadêmicos — PUC Minas](https://github.com/LuidiC)
> Sistemas desenvolvidos em equipe durante o curso de Engenharia de Software, com aplicação de Scrum, engenharia de requisitos e modelagem UML. Projetos com foco em problemas reais, incluindo temas de impacto como Green Computing e gestão de e-waste.

![Scrum](https://img.shields.io/badge/Scrum-0a192f?style=flat-square&logoColor=64ffda)
![UML](https://img.shields.io/badge/UML-0a192f?style=flat-square&logoColor=64ffda)
![Engenharia de Requisitos](https://img.shields.io/badge/Requisitos-0a192f?style=flat-square&logoColor=64ffda)

---

### 🐍 [Estudos em Python & Ecossistema de Dados](https://github.com/LuidiC)
> Repositório de aprofundamento técnico em Python — desde fundamentos até frameworks modernos. Cobre manipulação de dados com Pandas, construção de APIs com FastAPI e algoritmos aplicados à otimização de recursos.

![Python](https://img.shields.io/badge/Python-0a192f?style=flat-square&logo=python&logoColor=64ffda)
![Pandas](https://img.shields.io/badge/Pandas-0a192f?style=flat-square&logo=pandas&logoColor=64ffda)
![FastAPI](https://img.shields.io/badge/FastAPI-0a192f?style=flat-square&logo=fastapi&logoColor=64ffda)

---

## 📊 GitHub Stats

<div align="center">

<img width="48%" src="https://github-stats-alpha.vercel.app/api?username=LuidiC&cc=0a192f&tc=64ffda&ic=64ffda&bc=0a192f" />

<img width="48%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=LuidiC&layout=compact&hide_border=true&bg_color=0a192f&title_color=64ffda&text_color=8892b0" />

<br><br>

<img width="60%" src="https://streak-stats.demolab.com?user=LuidiC&theme=tokyonight-duo&hide_border=true&background=0A192F&stroke=64FFDA&ring=64FFDA&fire=64FFDA&currStreakLabel=64FFDA&sideLabels=8892B0&dates=8892B0&currStreakNum=64FFDA&sideNums=64FFDA" />

</div>

---

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:112240,50:0a192f,100:0d1117&height=120&section=footer" />

<sub>Aberto a conexões, troca de conhecimentos e projetos de automação — sinta-se à vontade para entrar em contato.</sub>

</div>
